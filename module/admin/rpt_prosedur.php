...
<script>
  $(document).ready(function() {
    ...
    const apiUrl = 'controller/report/prosedurController.php';
    ...
    var table = $('#periodeTable').DataTable({
      ...
      footerCallback: function(row, data, start, end, display) {
        ...
        data.forEach(row => {
          totalPasien += parseInt(row.total_kunjungan) || 0;
          feeDokter += parseInt(row.fee_dokter) || 0;
          feePerawat += parseInt(row.fee_perawat) || 0;
          feeAdmin += parseInt(row.fee_admin) || 0;
        });
        ...
        // isi footer
        $(this.api().column(1).footer()).html(totalPasien);
        $(this.api().column(2).footer()).html(formatRupiah(feeDokter));
        $(this.api().column(3).footer()).html(formatRupiah(feePerawat));
        $(this.api().column(4).footer()).html(formatRupiah(feeAdmin));
      }
    });
    ...
    $('#btnExcel').on('click', function() {
      ...
      let url = `module/report/export_prosedur.php?fromDate=${fromDate}&toDate=${toDate}&doctor=${doctor}&provider=${provider}`;
