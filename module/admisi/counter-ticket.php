    ...
    <div class="card kiosk-card">
      <div class="card-body p-4">
        ...
        <div class="mb-4">
          ...
          <select id="poli" class="form-select form-select-lg" required>
            <option value="">PILIH POLIKLINIK</option>
            <?php
            $query = tampildata("SELECT * FROM ms_poli WHERE poli_status='1' ")
            ?>
            <?php foreach ($query as $rows): ?>
              <option value="<?= $rows['id_poli'] ?>"><?= $rows['poli_name'] ?></option>
            <?php endforeach ?>
          </select>
        </div>
