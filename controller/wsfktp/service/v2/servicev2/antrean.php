...
if ($cek->num_rows > 0) {
    ...
    if ($status_pasien_baru == true) {
        ...
        try {
            ...
            $status_antrian = 0;
            $status_visit = '10';
            $stmt4 = $koneksi->prepare("INSERT INTO pasien_visit (id_patient,visit_ID,visit_date,id_poli,source_hub,created_user,visit_antrian,status_antrian,id_customer,visit_status,patient_name_pcare,noKartu, code_doctor) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt4->bind_param(
                "isssssiiissss",
                $id_patient,
                $visit_ID,
