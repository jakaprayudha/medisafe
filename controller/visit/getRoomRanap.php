...
switch ($type) {
   ...
   case 'room_name':
      $stmt = $koneksi->prepare("SELECT id_room, room_name FROM ms_room WHERE service_class = ? AND room_status = 1 AND id_customer = ? ORDER BY room_name ASC");
      $stmt->bind_param("si", $value, $id_customer);
      $stmt->execute();
      $result = $stmt->get_result();
      $data = [];
      while ($row = $result->fetch_assoc()) {
   ...
   case 'bed_name':
      $stmt = $koneksi->prepare("SELECT id_bed, bed_name, bed_gender FROM ms_room_bed WHERE id_room = ? AND bed_status = 0 AND id_customer = ? ORDER BY bed_name ASC");
