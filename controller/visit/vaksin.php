...
function createData()
{
   ...
   // Ambil semua field yang valid untuk tabel pasien_vaksin
   $allowedFields = [
      'id_visit',
      'vaksin_name',
      'vaksin_date',
   ...
   $fields = ['vaksin_number']; // tambahkan vaksin_number
   ...
   $query = "INSERT INTO pasien_vaksin ($columns) VALUES ($placeholders)";
...
/**
 * Generate vaksin_number unik dengan format DCT-XXXXXX
 */
function generateDoctorNumber($koneksi)
{
   ...
   do {
      ...
      $check = $koneksi->prepare("SELECT COUNT(*) FROM pasien_vaksin WHERE vaksin_number = ?");
...
function getData()
{
   ...
   $query = "SELECT * FROM pasien_vaksin WHERE id_visit = '$no'
             ORDER BY id_vaksin  ASC";
...
function  getID($iduser)
{
   ...
   $query = "SELECT * FROM pasien_vaksin   WHERE id_vaksin = ?";
...
function updateData()
{
   ...
   $query = "UPDATE pasien_vaksin SET " . implode(',', $fields) . " WHERE id_vaksin=?";
...
function deleteData()
{
   ...
   $query = "DELETE FROM pasien_vaksin WHERE id_vaksin = ?";
