...
function createData()
{
   ...
   // Ambil semua field yang valid untuk tabel visit_cppt_dokter
   $allowedFields = [
      'nomor_rm',
      'visit_ID',
      'cppt_date',
   ...
   $query = "INSERT INTO visit_cppt_dokter ($columns) VALUES ($placeholders)";
...
function generateDoctorNumber($koneksi)
{
   ...
   do {
      ...
      $check = $koneksi->prepare("SELECT COUNT(*) FROM visit_cppt_dokter WHERE cppt_number = ?");
...
function getData()
{
   ...
   $query = "SELECT * FROM visit_cppt_dokter WHERE visit_ID = '$no' AND nomor_rm = '$rm'
...
function  getID($iduser)
{
   ...
   $query = "SELECT * FROM visit_cppt_dokter   WHERE id_cppt = ?";
...
function updateData()
{
   ...
   $query = "UPDATE visit_cppt_dokter SET " . implode(',', $fields) . " WHERE id_cppt=?";
...
function deleteData()
{
   ...
   $query = "DELETE FROM visit_cppt_dokter WHERE id_cppt = ?";
