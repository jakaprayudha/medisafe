...
SELECT 
   ...
   CASE WHEN pp.id_visit IS NOT NULL THEN 1 ELSE 0 END AS ada_obat,
   CASE WHEN vi.id_visit IS NOT NULL THEN 1 ELSE 0 END AS ada_lab
...
LEFT JOIN (
   SELECT DISTINCT id_visit FROM permintaan_pharmacy
) pp ON pp.id_visit = pv.visit_ID

LEFT JOIN (
   SELECT DISTINCT id_visit FROM visit_inspection
