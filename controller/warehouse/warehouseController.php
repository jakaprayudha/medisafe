...
function createWarehouse()
{
   ...
   $warehouse = isset($_POST['gudang']) ? $_POST['gudang'] : '';
   $description = isset($_POST['description']) ? $_POST['description'] : '';

   if (empty($warehouse)) {
      echo json_encode([
         'status' => 'error',
         'message' => 'Nama Gudang harus diisi.'
      ]);
      exit;
   }

   // Query untuk insert data user
   $query = "INSERT INTO ms_warehouse (warehouse_name, warehouse_description) VALUES (?, ?)";
...
function getWarehouse()
{
   ...
   $query = "SELECT * FROM ms_warehouse";
   ...
   $totalQuery = "SELECT COUNT(*) AS total FROM ms_warehouse";
...
function getWarehouseID($iduser)
{
   global $koneksi;

   // Query untuk mengambil data user berdasarkan iduser
   $query = "SELECT * FROM ms_warehouse WHERE id_warehouse = ?";
...
function updateWarehouse()
{
   ...
   $warehouse = isset($_PUT['gudang']) ? $_PUT['gudang'] : '';
   $description = isset($_PUT['deskripsi']) ? $_PUT['deskripsi'] : '';

   // Debugging input data
   if (empty($warehouse) || empty($id)) {
      echo json_encode([
         'status' => 'error',
   ...
   $query = "UPDATE ms_warehouse SET warehouse_name = ?, warehouse_description = ? WHERE id_warehouse  = ?";

   if ($stmt = $koneksi->prepare($query)) {
      $stmt->bind_param("ssi", $warehouse, $description, $id);
      if ($stmt->execute()) {
         header('Content-Type: application/json');
         echo json_encode([
            'status' => 'success',
...
function deleteWarehouse()
{
   ...
   $query = "DELETE FROM ms_warehouse WHERE id_warehouse = ?";
